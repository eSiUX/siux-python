...
# generate date: 2015-15-22 10:15:07
...
class SiUXmethod:
	...
	def operatorLogList( self, client='', operatorId=0 ):
		return self._call( 'operator.log.list', client, operatorId )
	...
	def seleniumAdvancedAdd( self, client='', name='', desc='', code='', nickname='', inputUrl='', targetUrl='' ):
		return self._call( 'selenium.advanced.add', client, name, desc, code, nickname, inputUrl, targetUrl )
	...
	def sourceAllActive( self, client='' ):
		return self._call( 'source.all.active', client )

	def sourceAllDeactive( self, client='' ):
		return self._call( 'source.all.deactive', client )
